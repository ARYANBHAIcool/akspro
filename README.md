...
Standalone, automated live sports and 24/7 TV streaming web application modeled directly after **`ppv.st`**, **`damitv.st`**, and **`www.futbol-x.xyz`**.
...
- **Real-Time Automated Match Aggregator (`js/api.js`)**:
  ...
  - Powered by open, public APIs from `futbol-x.xyz` and `damitv.st` / `ppv.st`.
...
- **Universal Multi-Server Player Engine (`js/player.js`)**:
  - 1-click tab switching (`Server 1`, `Server 2`, `Server 3`, etc.) with labeled stream providers (e.g. `ESPN HD`, `Sky Sports`, `Main Feed`).
...
- **Cloudflare Pages Backend Proxy (`functions/api/damitv/`)**:
  - Serverless function proxy enabling seamless CORS handling on deployment.
